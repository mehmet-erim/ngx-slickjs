...
export interface USlickContainerDirective {
  ...
  slickContainerDivElement: HTMLDivElement;
...
describe('SlickContainerDirective', function(this: USlickContainerDirective) {
  describe('as a unit', () => {
    beforeEach(() => {
      ...
      this.slickContainerDivElement = this.fixture.debugElement.query(By.css('#slick-container')).nativeElement;
      this.fixture.detectChanges();
      this.lazyLoadService = this.fixture.debugElement.injector.get(LazyLoadService);
    ...
    it('should be jquery loaded', fakeAsync(() => {
      this.slickContainer.init.subscribe(() => {
        expect(document.querySelector(`[src='https://code.jquery.com/jquery-3.4.0.min.js']`)).toBeTruthy();
      });
      tick();
    }));
    ...
    it('should be slickJs loaded', fakeAsync(() => {
      this.slickContainer.init.subscribe(() => {
        expect(
          document.querySelector(`[src='https://cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.min.js']`),
        ).toBeTruthy();
      });
      tick();
    }));
    ...
    it('should be slick css loaded', fakeAsync(() => {
      this.slickContainer.init.subscribe(() => {
        expect(
          document.querySelector(`[href='https://cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.css']`),
        ).toBeTruthy();
      });
      tick();
    }));
    ...
    it('should be slick theme css loaded', fakeAsync(() => {
      this.slickContainer.init.subscribe(() => {
        expect(
          document.querySelector(`[href='https://cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick-theme.css']`),
        ).toBeTruthy();
      });
      tick();
    }));
    ...
    it('should emitted afterChange when change slide', fakeAsync(() => {
      this.slickContainer.afterChange.pipe(filter(({ currentSlide }) => currentSlide)).subscribe(res => {
    ...
    it('should emitted afterChange when next method worked', fakeAsync(() => {
      ...
      tick();
      this.slickContainer.afterChange.pipe(filter(({ currentSlide }) => currentSlide > 2)).subscribe(res => {
    ...
    it('should emitted afterChange when prev method worked', fakeAsync(() => {
      ...
      tick();
      this.slickContainer.afterChange.pipe(filter(({ currentSlide }) => currentSlide > 0)).subscribe(res => {
    ...
    it('should emitted afterChange when goTo method worked', fakeAsync(() => {
      ...
      tick();
      this.slickContainer.afterChange.pipe(filter(({ currentSlide }) => currentSlide === 3)).subscribe(res => {
        expect(res.currentSlide).toBe(3);
      });
      tick();
    }));
    ...
    it('should be changed the slide when dispatched mouse wheel event ', fakeAsync(() => {
      (this.component.config as Slick.Config).autoplay = true;
      this.fixture.detectChanges();

      this.slickContainer.init.subscribe(() => {
        // this.slickContainerDivElement.dispatchEvent(new)
      });
      tick();
      this.slickContainer.afterChange.pipe(filter(({ currentSlide }) => currentSlide > 0)).subscribe(res => {
        console.log(res);
        expect(this.slickContainerDivElement).toBeDefined();
      });
      tick();
    }));
  });
